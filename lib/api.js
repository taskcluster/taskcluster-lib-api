...
var schema = function(validate, options) {
  return function(req, res, next) {
    ...
    if (options.input !== undefined && !options.skipInputValidation) {
      ...
      if (error) {
        req.logger.error(error, 'Input schema validation error');
        return res.reportError(
    ...
    res.reply = function(json) {
      ...
      if (options.output !== undefined && !options.skipOutputValidation &&
        ...
        if (error) {
          req.logger.error(error, 'Output schema validation error: ');
...
/**
 * Attach a logger to the request.  This will later be bound to the context
 * object passed into the handler
 */
var attachLogger = function(logger) {
  return function(req, res, next) {
...
var handle = function(handler, context, name) {
  ...
  return function(req, res) {
    ...
    firstPromise.then(function(clientId) {
      ...
      req.logger.trace('calling request handler');
      let outcome = handler.call(_context, req, res);
      req.logger.trace('finished request handler');
      return outcome;
    }).then(function() {
      if (!req.hasAuthed) {
