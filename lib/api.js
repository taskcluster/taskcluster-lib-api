...
var Log = require('taskcluster-lib-log')

// This is a logger for general logging from this library, not logging content
// which matter to implementors of this library
var log = Log('taskcluster-lib-api');
...
var schema = function(validate, options) {
  return function(req, res, next) {
    ...
    if (options.input !== undefined && !options.skipInputValidation) {
      ...
      if (error) {
        log.error(error, 'Input schema validation error');
        return res.reportError(
          'InputValidationError',
          error,
          {schema: options.input, requestId: req.requestId});
      }
    }
    ...
    res.reply = function(json) {
      ...
      if (options.output !== undefined && !options.skipOutputValidation &&
        ...
        if (error) {
          log.error(error, 'Output schema validation error: ');
  ...
  return function(nonce, ts, cb) {
    for (var i = 0; i < N; i++) {
      if (noncedb[i].nonce === nonce && noncedb[i].ts === ts) {
        // NOTE: mozdef is not currently hooked up, but the idea is that this
        // is how we'd send specific messages to mozdef or a similar service
        log.error({mozdef: true}, 'CRITICAL: Replay attack detected!');
    ...
    try {
      ...
      req.authorize = async function(params) {
        ...
        if (process.env.NODE_ENV === 'production') {
          // Leaving in place because I really don't want to break a security system which might already
          // depend on this being there.
          console.log(`Authorized ${await req.clientId()} for ${req.method} access to ${req.originalUrl}`);
        }
        log.warn({
          clientId,
          method: req.method,
...
/**
 * Attach a requestId to each request and give that request ID.
 */
var attachRequestId = function(req, res, next) {
  if (!req.requestId) {
    let requestId = uuid.v4();
    req.requestId = requestId;
    res.set('X-Taskcluster-Request-Id', requestId); 
  } else {
    throw new Error('Already attached a requestId to this request');
  }
 ...
 */
var handle = function(handler, context, name, requestLogger) {
  assert(handler, 'No handler is provided');
  assert(requestLogger, 'No logger for request given');
  return function(req, res) {
    let firstPromise = Promise.resolve('unknown-client');

    // TODO: Figure out how to resolve the clientId here in a way that doesn't
    // cause a bunch of unit test failures, as happens when setting
    // firstPromise = req.clientId().

    firstPromise.then(function(clientId) {
      // We want to expose a log so that each request can do things like:
      // "log.info('did a thing')" that correctly prints out information about
      // the request, like which endpoint, etc
      let _logger = requestLogger.child({
      ...
      _logger.trace('calling request handler');
      let outcome = handler.call(_context, req, res);
      _logger.trace('finished request handler');
      return outcome;
    }).then(function() {
      if (!req.hasAuthed) {
 ...
 *   },
 *   logger:                          // (optional) A root logger or string.
 *                                    // If not specified, a root logger with
 *                                    // the name "taskcluster-lib-api" will be
 *                                    // used.  If an object, this will be the
 *                                    // actual root for the handler loggers.
 *                                    // If a string, it will be the name of the
 *                                    // taskcluster-lib-logger created
 * }
 *
...
var API = function(options) {
  ...
  if (options.logger) {
    if (typeof options.logger === 'string') {
      this._rootLogger = Log(options.logger);
    } else if (typeof options.logger === 'object') {
      this._rootLogger = options.logger;
    } else {
      throw new Error('The logger paramter must be a string or object');
    }
  } else {
    this._rootLogger = log;
  }
...
API.prototype.router = function(options) {
  ...
  _.concat(this._entries, [ping]).forEach(entry => {
    ...
    let endpointLogger = this._rootLogger.child({
      apiName: entry.name,
      method: entry.method,
      route: entry.route,
    });
    ...
    middleware.push(
      ...
      handle(entry.handler, options.context, options.name, endpointLogger)
    );
...
API.prototype.reference = function(options) {
  ...
  var schema = JSON.parse(fs.readFileSync(schemaPath, {encoding: 'utf-8'}));
  ...
  if (!valid) {
    ...
    this._rootLogger.error({reference, schema, errors: validate.errors}, 'api reference failed to validate against schema');
    let err = new Error('API.references(): Failed to validate against schema');
