...
let log = require('taskcluster-lib-log')('api');
...
let BuildReportErrorMethod = (method, errorCodes, monitor, cleanPayload) => {
  return (req, res, next) => {
    ...
    res.reportInternalError = (err, tags = {}) => {
      let requestId = req.requestId || uuid.v4();
      res.reportError(
        'InternalServerError',
        'Internal Server Error, incidentId: ' + requestId,
        {requestId}
      );
      log.info({
        err,
        url: req.url,
        requestId,
        incidentId: requestId, // DEPRECATED!
      }, 'error handling method');
      if (monitor) {
        err.requestId = requestId;
        err.incidentId = requestId; // DEPRECATED!
        err.method = method;
